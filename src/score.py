'''
Created on 09/4/2021

@author: Amazon Routing Challenge Community  
'''
import numpy as np
import json
import sys
...
def erp_per_edit_helper(actual,sub,matrix,g=1000,memo=None):
    ...
    if memo==None:
        memo={}
    actual_tuple=tuple(actual)# 不可变
    sub_tuple=tuple(sub)
    if (actual_tuple,sub_tuple) in memo:# 如果存有 则执行如下的
        d,count=memo[(actual_tuple,sub_tuple)]
        return d,count
    if len(sub)==0:
        d=gap_sum(actual,g)# 如果另一个恰好此时也为0 那么就是0，0然后输出
        count=len(actual)
    elif len(actual)==0:
        d=gap_sum(sub,g)
        count=len(sub)
        ...
        head_sub=sub[0]
        rest_actual=actual[1:]# 如果只有一个元素直接为空
        rest_sub=sub[1:]
        score1,count1=erp_per_edit_helper(rest_actual,rest_sub,matrix,g,memo)# 同时砍头
        score2,count2=erp_per_edit_helper(rest_actual,sub,matrix,g,memo)# 砍actual 跟完整sub 对比
        score3,count3=erp_per_edit_helper(actual,rest_sub,matrix,g,memo)# 砍submit  跟完整actual 比较
        option_1=score1+dist_erp(head_actual,head_sub,matrix,g)#直接给两个点的距离
        option_2=score2+dist_erp(head_actual,'gap',matrix,g)#加上1000
        option_3=score3+dist_erp(head_sub,'gap',matrix,g)# 加上1000 肯定是第一个小啊
        d=min(option_1,option_2,option_3)
        if d==option_1:
            if head_actual==head_sub:
                count=count1
            else:
                count=count1+1# 不是同一个点那就计数加一 此时不论偏离多少了
        elif d==option_2:
            count=count2+1
        else:
            count=count3+1
